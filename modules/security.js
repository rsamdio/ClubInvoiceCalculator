// Security and Validation Module (non-UI, behavior-preserving)
// Exposes: window.SecurityUtils, window.FormValidator
		...
		window.FormValidator = class FormValidator {
			constructor() {
				...
				this.validationRules = {
					'member-name': [
						{ test: (value) => window.SecurityUtils.validateMemberName(value), message: 'Member name is required and must be 2-100 characters with only letters, spaces, hyphens, apostrophes, and periods' }
					],
					'member-type': [
					...
					'join-date': [
						...
						{ test: (value) => window.SecurityUtils.validateDate(value), message: 'Please enter a valid date in YYYY-MM-DD format' }
					],
					'leave-date': [
						{ test: (value) => !value || window.SecurityUtils.validateDate(value), message: 'Please enter a valid date in YYYY-MM-DD format' },
			...
			setFieldError(fieldId, message) {
				...
				if (field && errorElement) {
					...
					errorElement.textContent = window.SecurityUtils.sanitizeText(message);
